...
`paste` is a very powerful function, but we can see that it has added in an extra spaces before the comma. This is because of a default argument of `paste`. We can get around that with `paste0`, which has zero spaces between phrases.
...
Now let's put this into action in the function:
...
This package is looking pretty nice - let's see how close it is to being ready by running it through the gamut of `devtools::check()`
